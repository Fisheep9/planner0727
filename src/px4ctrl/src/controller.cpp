    ...
    double voltage)
{
  ...
  const Eigen::Vector3d pid_error_accelerations = computePIDErrorAcc(odom, des, param); //位置环+速度环
  Eigen::Vector3d translational_acc = pid_error_accelerations + des.a; //速度环控制器输出+前馈加速度
  Eigen::Quaterniond desired_attitude, idel_att;
  Eigen::Vector3d omega;
  double thrust, debug_thrust;
  translational_acc = (Gravity + computeLimitedTotalAccFromThrustForce(translational_acc - Gravity, 1.0)).eval(); //限幅
  ...
  minimumSingularityFlatWithDrag(param.mass, param.gra, //姿态环
                                 des.v, translational_acc, des.j, des.yaw, des.yaw_rate,
                                 odom.q, desired_attitude, u.bodyrates, thrust); //计算期望姿态desired_attitude、推力thrust、（角速度u.bodyrates）

  Eigen::Vector3d thrustforce = desired_attitude * (thrust * Eigen::Vector3d::UnitZ()); //thrust需要转成向量再转到world系下（Eigen库会自动将三维向量转换成四元向量）
  Eigen::Vector3d total_des_acc = computeLimitedTotalAccFromThrustForce(thrustforce, param.mass); //（world系）推力转成（world系）加速度+限幅

  u.thrust = computeDesiredCollectiveThrustSignal(odom.q, odom.v, total_des_acc, param, voltage); // 期望（world系）加速度+推力模型算出期望油门值（0~1）的映射关系

  const Eigen::Vector3d feedback_bodyrates = computeFeedBackControlBodyrates(desired_attitude, odom.q, param); // 角度环（输入是世界系 会转到body系）
  ...
  // imu.q 是惯性系到body系
  // odom.q.inverse() 是动捕系到惯性系
  // desired_attitude 是动捕系
...
bool Controller::flatnessWithDrag(const Eigen::Vector3d &vel, //根据当前速度、加速度、加速度变化率和偏航角，计算出带有阻力效应的平坦模型的期望姿态、角速度和推力。
...
void Controller::minimumSingularityFlatWithDrag(const double mass, //最小化奇异性
                                                const double grav,
                                                ...
                                                double &thrust) const
{
  ...
  Eigen::Vector4d quat;
  if (flatnessWithDrag(vel, acc, jer, yaw, yawd, // 计算期望姿态quat（微分平坦）、角速度omg和推力thrust （这个微分平坦函数还没看）
                       thrust, quat, omg,
...
Eigen::Vector3d Controller::computeFeedBackControlBodyrates( 
    const Eigen::Quaterniond &des_q, //世界系
    const Eigen::Quaterniond &est_q, //世界系
    const Parameter_t &param)
{
  // Compute the error quaternion
  const Eigen::Quaterniond q_e = est_q.inverse() * des_q;  //转body系

  Eigen::AngleAxisd rotation_vector(q_e); //将四元数表示的旋转转换为轴角式
  Eigen::Vector3d axis = rotation_vector.axis();  //旋转（轴）向量
  debug.err_axisang_x = axis(0);
  debug.err_axisang_y = axis(1);
  debug.err_axisang_z = axis(2);
  debug.err_axisang_ang = rotation_vector.angle(); //旋转角
  ...
  if (q_e.w() >= 0)
  {
    bodyrates.x() = 2.0 * KAng(0) * q_e.x(); // 2.0 * q_e.x() 就是body系下的角度差（这里有小角度近似）
    ...
    double voltage)
{
  ...
  if (param.thr_map.accurate_thrust_model) //使用精确模型 accurate_thrust_model
  {
