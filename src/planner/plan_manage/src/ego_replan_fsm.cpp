...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    cout<<"[init waypoints] waypoints num:"<< waypoint_num_ << endl;
    for (int i = 0; i < waypoint_num_; i++)
    {
      nh.param("fsm/waypoint" + to_string(i) + "_x", waypoints_[i][0], -1.0);
      nh.param("fsm/waypoint" + to_string(i) + "_y", waypoints_[i][1], -1.0);
      nh.param("fsm/waypoint" + to_string(i) + "_z", waypoints_[i][2], -1.0);
      cout<<"[init waypoints] ("<<waypoints_[i][0] << ", " << waypoints_[i][1] << ", " << waypoints_[i][2] << ")" << endl;
    }

    // nh.param("fsm/waypoint_num", waypoint_num_, -1); //INIT直接从参数服务器读 后面改参数后需要重新读
    // for (int i = 0; i < waypoint_num_; i++)
    // {
    //   nh.param("fsm/waypoint" + to_string(i) + "_x", waypoints_[i][0], -1.0);
    //   nh.param("fsm/waypoint" + to_string(i) + "_y", waypoints_[i][1], -1.0);
    ...
    bspline_pub_ = nh.advertise<traj_utils::Bspline>("planning/bspline", 10); //把规划好的bspline轨迹发出去
    ...
    {
      trigger_sub_ = nh.subscribe("/traj_start_trigger", 1, &EGOReplanFSM::triggerCallback, this); // px4ctrl在AUTO_HOVER下发来trigger（6通打上去再打回来）就开始规划（规划器再发轨迹给px4ctrl执行）
  ...
  void EGOReplanFSM::readGivenWps()
  {
    ...
    if(call_count == 1){
      cout << "[Traj] call_count: " << call_count << endl;
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      /*** FSM ***/
      if (exec_state_ == WAIT_TARGET)
        changeFSMExecState(GEN_NEW_TRAJ, "TRIG"); //GEN_NEW_TRAJ内会继续规划局部轨迹
      else
      {
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      bool success = planFromGlobalTraj(10); // zx-todo replan 在全局轨迹基础上继续规划局部轨迹
      if (success)
      {
        changeFSMExecState(EXEC_TRAJ, "FSM"); // 规划成功就EXEC_TRAJ
    ...
    case EXEC_TRAJ:
    {
      ...
      double t_cur = (time_now - info->start_time_).toSec(); //轨迹开始时间
      t_cur = min(info->duration_, t_cur);

      Eigen::Vector3d pos = info->position_traj_.evaluateDeBoorT(t_cur); //当前时间点在轨迹上的位置

      /* && (end_pt_ - pos).norm() < 0.5 */

      // 如果不是最后一个wp且当前轨迹剩余距离小于1m  就plan下一个wp
      if ((target_type_ == TARGET_TYPE::PRESET_TARGET) &&
          (wp_id_ < waypoint_num_ - 1) &&
          (end_pt_ - pos).norm() < no_replan_thresh_) 
      {
        wp_id_++;
        planNextWaypoint(wps_[wp_id_]); //如果成功则 end_pt_ = next_wp;
      }
      // 如果下一个局部规划目标是wp
      else if ((local_target_pt_ - end_pt_).norm() < 1e-3) 
      {
        if (t_cur > info->duration_ - 1e-2) // 如果时间快分配完了？说明这是最后一个wp
        {
          ...
          ROS_WARN("Traj Plan done! All waypoints have been reset!");
        ...
        else if ((end_pt_ - pos).norm() > no_replan_thresh_ && t_cur > replan_thresh_) //距离太远 继续replan
        {
          changeFSMExecState(REPLAN_TRAJ, "FSM");
        }
      }
  ...
  bool EGOReplanFSM::planFromGlobalTraj(const int trial_times /*=1*/) //zx-todo 在全局轨迹基础上继续规划局部轨迹
  {
    start_pt_ = odom_pos_; //当前odom
    ...
    {
      if (callReboundReplan(true, flag_random_poly_init)) //规划局部轨迹
      {
        return true;
      }
    }
  ...
  void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
  {
    ...
    Eigen::Vector3d p_cur = info->position_traj_.evaluateDeBoorT(t_cur); //当前位置信息
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj) //规划局部轨迹
  {

    getLocalTarget(); //在全局轨迹上搜索局部目标点并计算局部目标速度 (wp之间的距离大于6m才需要插入新的local_target_pt_ 要不然local_target_pt_就是下一个wp)


    std::cout << "local_target_pt_: (" << local_target_pt_.x() << ", " << local_target_pt_.y() << ", " << local_target_pt_.z() <<")"<< std::endl;
    std::cout << "local_target_vel_: (" << local_target_vel_.x() << ", " << local_target_vel_.y() << ", " << local_target_vel_.z() <<")"<< std::endl;


    bool plan_and_refine_success =
        planner_manager_->reboundReplan(start_pt_, start_vel_, start_acc_, local_target_pt_, local_target_vel_, (have_new_target_ || flag_use_poly_init), flag_randomPolyTraj);
    have_new_target_ = false; //规划局部轨迹并优化、细化（这个函数是Bspline优化重点）
  ...
  void EGOReplanFSM::getLocalTarget()  //在全局轨迹上搜索局部目标点并计算局部目标速度
  {
    double t;

    double t_step = planning_horizen_ / 20 / planner_manager_->pp_.max_vel_; //时间步长，以便在全局轨迹上迭代搜索局部目标点
    double dist_min = 9999, dist_min_t = 0.0;
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      Eigen::Vector3d pos_t = planner_manager_->global_data_.getPosition(t); //时间 t 对应的全局轨迹点的位置 pos_t
      double dist = (pos_t - start_pt_).norm(); //起点到 pos_t的距离（start_pt_应该是每段局部轨迹的起点）
      ...
      if (dist >= planning_horizen_) // wp之间的距离大于6m才需要插入新的local_target_pt_ 要不然local_target_pt_就是下一个wp
      {
    ...
    if (t > planner_manager_->global_data_.global_duration_) // Last global point 当前（局部规划）时间超过上一个全局时间（也就是下一个最近的wp）则设定下一个局部目标end_pt_
    {
      local_target_pt_ = end_pt_; //end_pt_就是下一个wp
      planner_manager_->global_data_.last_progress_time_ = planner_manager_->global_data_.global_duration_;
    }

    if ((end_pt_ - local_target_pt_).norm() < (planner_manager_->pp_.max_vel_ * planner_manager_->pp_.max_vel_) / (2 * planner_manager_->pp_.max_acc_))
    {
      local_target_vel_ = Eigen::Vector3d::Zero(); //如果规划的局部目标太靠近wp 则经过速度为0
    }
    else
    {
