...
void PX4CtrlFSM::process()
{
	...
	Controller_Output_t u; // 控制器输出：姿态q、推力T、角速度（为什么要角速度）
	...
	case MANUAL_CTRL:
	{
		...
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			{
				if (!rc_data.is_hover_mode || !rc_data.is_command_mode || !rc_data.check_centered()) //offboard+px4ctrl+油门中位
				{
					ROS_ERROR("is_hover_mode: %d, is_command_mode: %d",rc_data.is_hover_mode,rc_data.is_command_mode);
	...
	case AUTO_HOVER:
	{
		...
		else if (rc_data.is_command_mode && cmd_is_received(now_time)) //收到plannar规划出的轨迹后 切为CMD_CTRL
		{
		...
		else // 悬停后直接告诉plannar开始规划
		{
			set_hov_with_rc(); // 直接打遥控
			des = get_hover_des(); //先定在原地
			if ((rc_data.enter_command_mode) || //如果是切过来的（6通打回去再打回来）
				(takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second)) 
			{
				takeoff_land.delay_trigger.first = false;
				publish_trigger(odom_data.msg);//告诉 ego_replan_fsm 开始规划轨迹（odom_data直接拿的/ekf/ekf_odom只是经过了处理）
	...
	case CMD_CTRL:
	{
		...
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time)) //轨迹发完后 没有新cmd就切回AUTO_HOVER悬停
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
	...
	case AUTO_TAKEOFF:
	{
		...
		{
			des = get_rotor_speed_up_des(now_time); //电机开转至起飞前 des只给期望的垂直加速度
		}
		...
		{
			des = get_takeoff_land_des(param.takeoff_land.speed); //起飞后达到指定高度前 des给期望的速度和位置（匀速）
		}
	...
	{
		switch (param.pose_solver) //姿态解算
		{
		case 0:
			debug_msg = controller.update_alg0(des, odom_data, imu_data, u, bat_data.volt); // 里面是 位置环+速度环+姿态+(角度环)
	...
	{
		publish_bodyrate_ctrl(u, now_time); // u:角度环输出(角速度,T)
	}
	else
	{
		publish_attitude_ctrl(u, now_time); // u:姿态输出（q,T）
	}
