...
namespace ego_planner
{
  ...
  bool EGOPlannerManager::reboundReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel, //规划局部轨迹并优化
                                        ...
                                        
  {
    ...
    if ((start_pt - local_target_pt).norm() < 0.2) //起点到local_target不能太近
    {
      cout << "[start_pt] close to [local_target_pt]" << endl;
    ...
    UniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts); // 参数化到 B 样条曲线

    vector<std::pair<int, int>> segments;
    segments = bspline_optimizer_->initControlPoints(ctrl_pts, true); // 初始化控制点
    ...
    if (pp_.use_distinctive_trajs) //多条轨迹优化
    {
      // cout << "enter" << endl;
    ...
    UniformBspline pos = UniformBspline(ctrl_pts, 3, ts); // 优化成功
    pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, pp_.feasibility_tolerance_); // 物理限制
    ...
    if (pp_.drone_id <= 0)
    {
      ...
      if (!pos.checkFeasibility(ratio, false))
      {
        ...
        if (flag_step_2_success)
          pos = UniformBspline(optimal_control_points, 3, ts);  // 优化成功
      }
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    PolynomialTraj gl_traj;
    if (pos.cols() >= 3) //起点+中间点+终点 数量>=3
      gl_traj = PolynomialTraj::minSnapTraj(pos, start_vel, end_vel, start_acc, end_acc, time); //多项式轨迹
    else if (pos.cols() == 2) 
      gl_traj = PolynomialTraj::one_segment_traj_gen(start_pos, start_vel, start_acc, end_pos, end_vel, end_acc, time(0)); //单段轨迹生成方法
    else
      return false;

    auto time_now = ros::Time::now();
    global_data_.setGlobalTraj(gl_traj, time_now); //这里设置全局轨迹的时间global_duration_
