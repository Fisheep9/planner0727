...
namespace ego_planner
{
  ...
  public:
    UniformBspline() {}
    UniformBspline(const Eigen::MatrixXd &points, const int &order, const double &interval); // 初始化为均匀 B-样条
    ~UniformBspline();
    ...
    Eigen::VectorXd evaluateDeBoor(const double &u); //计算控制点位置                               // use u \in [up, u_mp]
    inline Eigen::VectorXd evaluateDeBoorT(const double &t) { return evaluateDeBoor(t + u_(p_)); } // use t \in [0, duration]
    UniformBspline getDerivative(); //计算导数
    ...
    static void parameterizeToBspline(const double &ts, const vector<Eigen::Vector3d> &point_set,
                                      ...
                                      Eigen::MatrixXd &ctrl_pts); // 三维 B-样条插值，考虑边界速度和加速度约束
